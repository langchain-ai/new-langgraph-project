...
3. **Agent** получает результаты анализа через `get_case_details`
4. **Agent** выполняет действия на основе анализа

## 🛠️ Инструменты агента

Агент имеет доступ к 5 инструментам для работы с кейсами:

| Инструмент | Описание |
|------------|----------|
| `get_case_details` | Получение информации о кейсе (включая результаты анализа) |
...
1. Вызвать `get_case_details` для получения контекста
2. Изучить поле `analysis` с результатами анализа
3. Проанализировать историю диалога
